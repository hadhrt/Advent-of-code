...
def primeFactors(N):
    ...
    if N > 1:
        yield N  # remaining value is a prime if not 1
...
def p1(lines):
    ...
    id_ranges = [
        tuple(map(int, (range_string.split("-")))) for range_string in range_strings
    ]
    ...
    for start_ID, end_ID in id_ranges:
        for id in range(start_ID, end_ID + 1):
            ...
            for factor in unique_prime_factors:
                sequence_repeating_number = int(
                    str(id)[: len(str(id)) // factor] * factor
                )
